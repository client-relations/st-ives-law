export const WEBHOOK_URL =
  ...
  "https://hook.eu2.make.com/fou12e2mjy2wgv2h0e3jgqor7fu81rec";

// ─── Person mapper (used for executors, attorneys, guardians) ─────────────────
...
// ─── Financial sub-object ─────────────────────────────────────────────────────
function buildFinancial(prefix, aData) {
  const dl = aData.disclosureLevel || "";
  const isSummary = dl === "summary";
  return {
    disclosure_level: dl,
    properties: (aData[`${prefix}Props`] || []).map(p => ({
      address: p.address || "",
      type: p.type || "",
    ...
    bank_accounts: (aData[`${prefix}Banks`] || []).map(b => ({
      institution: b.institution || "",
      account_type: b.accountType || "",
      balance: b.balance || "",
      ownership: b.ownership || "",
    })),
    shares: (aData[`${prefix}Shares`] || []).map(s => ({
      platform: s.platform || "",
    ...
    foreign_assets: aData[`${prefix}ForeignAssets`] === "Yes"
      ...
      : null,
    superannuation: (aData[`${prefix}Supers`] || []).map(s => ({
      fund: s.fund || "",
      abn: s.abn || "",
      balance: s.balance || "",
      bdbn: s.bdbn || "",
    ...
    mortgages: (aData[`${prefix}Morts`] || []).map(m => ({
      lender: m.lender || "",
      secured_property: m.securedProperty || "",
      balance: m.balance || "",
      rate_type: m.rateType || "",
    })),
    loans: (aData[`${prefix}Loans`] || []).map(l => ({
      lender: l.lender || "",
      type: l.type || "",
      balance: l.balance || "",
    })),
...
// ─── Business / insurance sub-object ─────────────────────────────────────────
function buildBusiness(prefix, bData) {
  return {
    businesses: (bData[`${prefix}Businesses`] || []).map(b => ({
      name: b.name || "",
      structure: b.structure || "",
      acn_abn: b.acnAbn || "",
      value: b.value || "",
    })),
    trusts: (bData[`${prefix}Trusts`] || []).map(t => ({
      name: t.name || "",
      type: t.type || "",
      value: t.value || "",
    })),
    ...
    insurance: {
      ...
      life_tpd: (bData[`${prefix}Life`] || []).map(p => ({
        insurer: p.insurer || "",
        sum_insured: p.cover || "",          // field name in usePartB is 'cover'
        inside_super: p.heldInSuper || "",   // field name in usePartB is 'heldInSuper'
        binding_nomination: p.beneFixed || "", // field name in usePartB is 'beneFixed'
      })),
...
// ─── Will sub-object ──────────────────────────────────────────────────────────
function buildWill(prefix, cData) {
  return {
    has_existing_will: cData[`${prefix}HasExistingWill`] || "",
    existing_will_date: cData[`${prefix}WillDate`] || "",
    existing_will_location: cData[`${prefix}WillKept`] || "",  // where existing will is kept
    will_storage_preference: cData[`${prefix}WillLoc`] || "",  // where new will should be stored
...
// ─── EPA sub-object ───────────────────────────────────────────────────────────
function buildEpa(prefix, dData) {
  return {
    ...
    financial_epa_trigger: dData[`${prefix}EpaFinTrigger`] || "", // exact string preserved
...
// ─── Funeral sub-object ───────────────────────────────────────────────────────
function buildFuneral(prefix, dData) {
  return {
    burial_preference: dData[`${prefix}BurialPref`] || "",       // exact string preserved
...
// ─── Full client object ───────────────────────────────────────────────────────
function buildClient(prefix, aData, bData, cData, dData) {
  ...
  return {
    ...
    // c2 does not have litigation / additional_info fields in dData
...
export function buildFinalPayload(aData, bData, cData, dData, sessionId) {
  // Only explicit couple selections produce a client_2 object
  const isCouple =
    aData.engType === "Couple (Joint Matter)" ||
    aData.engType === "Couple (Separate Matters)";

  // Children: if sameChildren = 'No' on a couple, merge the two separate lists
  const children =
  ...
  return {
    ...
    engagement: {
      type: aData.engType || "",           // "Single Client" / "Couple (Joint Matter)" / "Couple (Separate Matters)"
      state: aData.state || "",
      urgent: aData.urgent || "",
...
// ─── Submission function ──────────────────────────────────────────────────────
export async function submitFinalWebhook(aData, bData, cData, dData, sessionId) {
  const payload = buildFinalPayload(aData, bData, cData, dData, sessionId);
